...
try:
    from walking.calibration_manager import CalibrationManager
...
class StabilizationModule:
    ...
    def __init__(self, config_loader=None):
        ...
        # Загружаем калиброванный PID из отдельного файла (не трогаем walking.yaml)
        calibrated_config = self.config.load('walking_pid')
        calibrated_pid = calibrated_config.get('stabilization_pid', {}) if isinstance(calibrated_config, dict) else {}
        ...
        calibrated_kp = calibrated_pid.get('calibrated_kp', pid_config.get('calibrated_kp'))
        calibrated_ki = calibrated_pid.get('calibrated_ki', pid_config.get('calibrated_ki'))
        calibrated_kd = calibrated_pid.get('calibrated_kd', pid_config.get('calibrated_kd'))
        ...
        if calibrated_kp is not None and not self.calibration_enabled:
            ...
            # Если откалиброванных нет и калибровка выключена — отключаем PID и предупреждаем
            if not self.calibration_enabled and calibrated_kp is None:
                self.pid_enabled = False
                rospy.logwarn("PID stabilization disabled: no calibrated parameters found. Enable calibration to generate them.")
                kp = ki = kd = 0.0
            else:
        ...
        # Инициализируем менеджер калибровки, если включена
        if self.calibration_enabled and CALIBRATION_AVAILABLE and self.pid_enabled and self.pid_controller is not None:
            self.calibration_manager = CalibrationManager(
                config_loader=self.config,
                pid_controller=self.pid_controller,
                target_angle=self.calibration_target_angle,
                angle_range=self.calibration_angle_range,
                output_limits=self.output_limits,
                start_delay=self.calibration_delay,
                duration=self.calibration_duration,
                setpoint_duration=self.calibration_setpoint_duration,
                calibrated_config_name='walking_pid'
            )
            # Callback для обновления параметров после калибровки
            self.calibration_manager.on_completed = self._on_calibration_completed
            self.calibration_manager.on_failed = self._on_calibration_failed
    ...
    def process(
        ...
        if self.pid_enabled and self.pid_controller is not None:
            # Обработка калибровки - обновление setpoint
            if self.calibration_manager is not None:
                current_time = time.time()
                # Обновляем setpoint калибровки
                self.calibration_manager.update_setpoint(current_time)
            ...
            if self.calibration_manager is not None:
                self.calibration_manager.collect_data_point(pitch_deg, pid_output)
    ...
    def _on_calibration_completed(self, result):
        """
        Callback вызывается при успешном завершении калибровки.
        
        Args:
            result: CalibrationResult с откалиброванными параметрами
        """
        # Обновляем PID контроллер с новыми параметрами СРАЗУ после калибровки
        ...
        # Логируем применение новых параметров
        rospy.loginfo(f"Applied calibrated PID parameters: Kp={result.kp:.6f}, Ki={result.ki:.6f}, Kd={result.kd:.6f}")
    
    def _on_calibration_failed(self, error_msg: str):
        """
        Callback вызывается при ошибке калибровки.
    ...
    def get_calibration_status(self):
        ...
        """
        if self.calibration_manager is not None:
            return self.calibration_manager.get_state()
        return 'idle'
